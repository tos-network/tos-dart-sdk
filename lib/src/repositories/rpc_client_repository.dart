...
sealed class RpcClientRepository {
  ...
  late Map<TosJsonKey, List<Function>> eventsCallbacks;
  ...
  Future<void> subscribeTo(TosJsonKey event) async {
    _logInfo('subscribing to ${event.jsonKey}...');
    // Wait until a connection has been established.
    await socket?.connection.firstWhere(
      (state) => state is Connected || state is Reconnected,
    );
    _send(TosSubscription.subscribe, {'notify': event.jsonKey});
  }

  /// Unsubscribe from a TOS event.
  Future<void> unsubscribeFrom(TosJsonKey event) async {
    _logInfo('unsubscribing from ${event.jsonKey}...');
    // Wait until a connection has been established.
    await socket?.connection.firstWhere(
      (state) => state is Connected || state is Reconnected,
    );
    _send(TosSubscription.unsubscribe, {'notify': event.jsonKey});
    eventsCallbacks[event]?.clear();
  }

  /// Registers a callback for a TOS event.
  void registerCallback(TosJsonKey event, Function callback) {
  ...
  void _send(TosJsonKey method, [Map<String, dynamic>? params]) {
    _id++;
  ...
  String _jsonRequest(TosJsonKey method, [Map<String, dynamic>? params]) {
    if (params != null) {
      return jsonEncode(
        {
          'id': _id,
